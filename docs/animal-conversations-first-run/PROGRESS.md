...
- **Next:** obtain real review corrections/approval, repair the observed CI compiler-version mismatch, finish supported-system checks, then render/review/export the real episodes. Never use synthetic approval/playback attestations as evidence for real episodes.
...
| Reliable setup | Node-only doctor before installation; normal rendering without Python/Cargo; pinned optional intake/converter dependencies | Fresh extracted Mac doctor correctly identified only missing Sharp before npm ci; doctor/check passed afterward. Default suite: 98 pass, 8 opt-in skips with Python/Cargo unavailable. Actual isolated Python/model setup passed | Mac fresh-artifact proof passed using caches; no cold model-download claim. Linux/WSL CI compiler mismatch being repaired |
...
| Supported systems | Fresh-install/end-to-end proof on macOS ARM64, Linux x64 and WSL Linux tooling | Mac artifact proof passed through real review. Real GitHub-hosted Linux and actual Ubuntu/WSL2 jobs ran: 106 Node + 3 Python passed; optional Rust converter compilation exposed CI's older Rust 1.85. WSL2 kernel positively verified | Partial, not certified; repair compiler setup and complete remaining checks |
...
| AC2 | Prepared offline intake; missing-model setup state; clear mocked URL failures | Actual helper/offline inference, fresh cached-model intake setup and mocked errors passed locally; Linux/WSL pending |
...
| Platforms | macOS ARM64 + Linux x64 + WSL using Linux binaries | Mac partial acceptance; Linux/WSL 106 core + 3 Python tests passed but optional converter CI compiler failed. No complete platform claim |
...
- Linux/WSL environments now exist in isolated read-only GitHub Actions. Initial run `33939590840` exposed Rust 1.85 being too old for the locked converter dependency. This is an observed CI toolchain repair, not permission to skip converter tests.
