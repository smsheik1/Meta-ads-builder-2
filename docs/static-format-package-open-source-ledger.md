...
| Semantic reference analysis | Gemma 4 31B IT through OpenRouter, pinned to DeepInfra | Selected Maker transport; provider fallbacks disabled | A real Codex reference passed the six-key schema and local reference validation in 38.2s after the public NVIDIA NIM endpoint repeatedly returned zero bytes | Keep invalid cross-references fail-visible, use only the six-key MVP contract, and retain Maker correction for partial but valid drafts |
...
Gemma 4 31B IT through OpenRouter's pinned DeepInfra backend is the Maker MVP semantic selection. The six-key contract plus local validation accepted the real Codex reference in 38.2 seconds during the provider gate and again in 54.2 seconds through the production helper. Maker correction remains necessary for partial but structurally valid drafts, while invalid references still fail visibly. Keep Gemma 26B as benchmark evidence, never as a silent fallback.
