...
| Semantic reference analysis | [Gemma 4 31B IT](https://build.nvidia.com/google/gemma-4-31b-it) | Provisional semantic lead; [minimal contract NIM smoke complete](./research-intake/minimal-maker-analysis-nim-2026-07-11.md) | The same Codecademy reference that timed out under v1.1 completed in 59.979 seconds with 709 prompt and 625 completion tokens; strict minimal schema, formula, three-item benefit List, and locked collage passed | Native CTA boundary, logo classification, active-item evidence, questions, and reroll coherence remain partial; use only the six-key MVP contract |
...
Gemma 4 31B IT is again the provisional semantic lead, not the production selection. Replacing the duplicated v1.1 request with the six-key Maker MVP contract reduced the Codecademy prompt by 83.5%, schema by 46.9%, and output ceiling by half; the previously timing-out NIM path returned valid output in 59.979 seconds. Semantic quality remained partial on native CTA, logo, active-item, question, and coherence decisions. Refine only those observed rules, then use genuinely fresh references. Keep Gemma 26B as benchmark evidence, never as a silent fallback.
