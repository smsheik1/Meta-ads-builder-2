...
- [Field + List corrected-contract smoke](./research-intake/static-reference-field-list-contract-smoke-2026-07-10.md)
...
| Semantic reference analysis | Gemma 4 31B IT through OpenRouter | Selected Maker transport; free route first, same-model host failover allowed | A real Codex reference passed the six-key schema on DeepInfra in 38.2s; the real Breaking News reference later returned 5 fields and 3 correct asset roles through the free route in 13.9s at zero cost | Require JSON-compatible hosts, keep invalid cross-references fail-visible, and never substitute a different model |
| Semantic reference analysis | [Gemma 4 26B A4B](https://replicate.com/prunaai/gemma-4-26b-a4b-fast) | Speed reference, not current lead; [Replicate benchmark complete](./research-intake/gemma26b-v12-regression-3-2026-07-11.md) | Warm inference completed in 11.955–20.276 seconds and recovered 3/3 formulas | Official v1.2 validity was 0/3 and full semantic acceptance was 0/3; do not add a runtime fallback |
...
- Multi-evidence Field grouping and logical List item accuracy
...
- Zero invalid active selections, duplicate active/supporting assignments, or silent List item loss in publishable drafts
- The Codex reference can become a coherent David's Cookies Format without changing its recognizable formula

### Model decision rule

Gemma 4 31B IT through OpenRouter is the Maker MVP semantic selection. The free route is tried first; compatible paid hosts may serve the exact same model if a host is unavailable. The six-key contract plus local validation accepted the real Codex reference on DeepInfra in 38.2 seconds and the real Breaking News reference through the free route in 13.9 seconds at zero OpenRouter cost. Maker correction remains necessary for partial but structurally valid drafts, while invalid references still fail visibly. Keep Gemma 26B as benchmark evidence, never as a silent fallback.

## 6. Secondary and Watchlist Candidates

| Candidate | Possible role | Why it is not primary today |
| --- | --- | --- |
| [Nemotron 3 Nano Omni 30B-A3B Reasoning](https://build.nvidia.com/nvidia/nemotron-3-nano-omni-30b-a3b-reasoning) | Semantic runner-up or later self-hosting comparison; [one legacy-contract smoke complete](./research-intake/vision-challengers-codex-smoke-2026-07-10.md) | Recognized the focal pair but returned internally inconsistent core membership, asked three unnecessary questions, took 86.0 seconds, and used 4,566 tokens; revised collection recovery remains untested |
| [Mistral Large 3 675B](https://build.nvidia.com/mistralai/mistral-large-3-675b-instruct-2512) | Fast semantic baseline; [one legacy-contract smoke complete](./research-intake/vision-challengers-codex-smoke-2026-07-10.md) | Completed in 9.8 seconds and returned all seven integrations together, but failed to represent `Slack` as the current active item and added three unnecessary questions; revised collection recovery remains untested |
...
3. One pure resolver for Field values, Lists, active/supporting assignment, visual policy, Reroll Groups, assets, and overrides.
...
| 2026-07-10 | Keep the corrected Field + List contract and stop tuning on the first five references | A second one-shot run produced 5/5 valid outputs, 3 passes, 2 partials, zero fake fragment Lists, and exactly five typed listicle rows; David's missed one scene and Justin's proof metrics were excluded | Add `evidence_bound`, retain Maker correction, test three to five untouched holdouts, and announce an exact SAM request count before any paid asset-localization run |
...
| 2026-07-11 | Mark the Codecademy final holdout inconclusive after provider 504 | Frozen expectations and 49 OCR regions exist, but NVIDIA returned HTTP 504 before any model output; no retry, fallback, repair, SAM, or Replicate call ran | Preserve the frozen request unchanged; require an explicit Try again and draw no semantic conclusion from the outage |
| 2026-07-11 | Stop unchanged Codecademy retries after a second provider 504 | The founder explicitly authorized one identical retry; it also failed before model output. Files were small, but 37 of 49 OCR regions were tiny text nested in the locked product collage | Keep semantic quality unscored; compact nested locked-raster OCR deterministically or use a non-gateway deployment of the same model before another validation |
| 2026-07-11 | Pass the narrow OCR evidence-compaction spike offline without changing v1.1 | One deterministic component reduced the Codecademy semantic evidence from 49 regions to 16 by replacing 34 tiny nested product-UI regions with one auditable cluster; five tests prove exact reversibility, protected copy, sparse no-op behavior, and a one-cluster ceiling | With explicit authorization, run one compacted Gemma request as a latency/path validation; a success still requires a genuinely fresh final semantic holdout |
| 2026-07-11 | Block the public Gemma NIM path after compacted evidence also returns 504 | The explicitly authorized compacted request used 16 rather than 49 evidence regions but still returned NVIDIA HTTP 504 after about 225 seconds before model output; no retry, fallback, repair, SAM, or Replicate ran | Keep the compactor as an offline pass, stop changing semantic contracts from output-free failures, and test the same model plus inputs on a non-gateway deployment |
...
| 2026-07-11 | Fail the three-reference Gemma 26B v1.2 regression gate and keep the model provisional | Three authorized pinned predictions ran with no retries or fallbacks; inference was 11.955–20.276 seconds, but all copied `$schema`, only the Meta Story passed v1.2 diagnostically, the comparison Lists were wrong, LinkedIn omitted six evidence owners, and asset discovery was incomplete | Remove schema metadata from the prompt projection offline without relaxing output validation; add repeated-set and duplicate-evidence fixtures; require a genuinely fresh gate before production selection or SAM |
| 2026-07-11 | Replace the overbuilt Field/List analysis request with one six-key Maker MVP contract | On the same compacted Codecademy reference, prompt characters fell 83.5%, schema characters 46.9%, and max output tokens 50%; Gemma 31B NIM completed in 59.979 seconds instead of 504, passed strict schema, found the three benefits, and locked the collage | Keep the minimal contract; refine only the five observed semantic misses before a fresh gate; do not restore confidence matrices, exclusion essays, speculative policies, retries, or fallbacks |
| 2026-07-11 | End prompt tuning after three known-ad regressions | All three Gemma 31B NIM requests completed in 25.100–104.604 seconds; LinkedIn recovered four coherent rows and Dumb vs Smart recovered both 3-item sets, but Codex still split active Slack from its List and Dumb vs Smart referenced an undeclared asset | Keep the model and minimal contract; build Maker correction for partial drafts, fail invalid cross-references visibly, and let the assistant's next naturally saved ad validate the real workflow |
| 2026-07-11 | Keep the live Maker route but block public-Gemma acceptance after three fail-visible timeouts | [Live route smoke](./research-intake/maker-live-analysis-route-smoke-2026-07-11.md): local PaddleOCR passed 5/5 references, two saved analyses normalized and passed browser QA, but three explicit Gemma 31B calls returned no output before the 300-second client limit; SAM ran zero times | Preserve the one-route, one-worker pipeline and saved provider fixture; do not add retries or fallbacks; retest the same model only after the public NIM endpoint recovers or on a non-gateway deployment |
| 2026-07-11 | Isolate the remaining timeout to the hosted Gemma model endpoint | NVIDIA's authenticated model catalog returned HTTP 200 and listed Gemma 4 31B, while a text-only eight-token request with no image or schema returned zero bytes before a 45-second timeout | Make no product changes; wait for the public model endpoint to recover or run the same model on a dedicated deployment |
| 2026-07-12 | Move Maker semantic analysis to exact Gemma 4 31B through OpenRouter's pinned DeepInfra backend | NVIDIA NIM again returned zero bytes after 120 seconds on an eight-token health prompt; OpenRouter returned in 1.97 seconds, pinned DeepInfra completed strict semantic validation in 38.2 seconds, and the production pipeline then completed PaddleOCR in 6.4s, Gemma in 40.5s, and one conditional SAM 3 prediction in 83.8s with zero warnings, 11 valid layers, all seven integrations, and Slack active | Disable provider fallbacks, keep the canonical schema unchanged, omit only provider-unsupported `uniqueItems` from the grammar copy, and continue to fail visibly without repair or retry |
