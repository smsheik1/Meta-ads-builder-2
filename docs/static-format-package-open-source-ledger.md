...
| Semantic reference analysis | [Gemma 4 31B IT](https://build.nvidia.com/google/gemma-4-31b-it) | Provisional semantic leader, pipeline not selected; [schema capability passed after the untouched holdout failed](./research-intake/nim-structured-output-capability-probe-2026-07-10.md) | Tuned corpus retained 5/5 formula recovery and zero fake fragment Lists; a model-specific `response_format.json_schema` probe passed strict parsing and Draft-07 validation | Recalibrate scope/confidence, then test new untouched holdouts; do not use unsupported top-level `guided_json` for this Gemma NIM variant |
...
Gemma 4 31B IT remains the provisional semantic benchmark leader, but the pipeline is not selected because the first untouched holdout gate failed. Provider-native schema enforcement is now proven using the Gemma variant's required `response_format.json_schema` transport; a top-level `guided_json` property was accepted but silently ignored. Test new untouched holdouts next. Only a repeatable semantic failure after schema enforcement justifies one challenger comparison; never ship multiple models as silent fallbacks.
