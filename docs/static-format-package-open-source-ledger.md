...
| 2026-07-12 | Move Maker semantic analysis to exact Gemma 4 31B through OpenRouter's pinned DeepInfra backend | NVIDIA NIM again returned zero bytes after 120 seconds on an eight-token health prompt; OpenRouter returned in 1.97 seconds, and pinned DeepInfra completed a real Codex image with strict structured output plus local validation in 38.2 seconds | Disable provider fallbacks, keep the canonical schema unchanged, omit only provider-unsupported `uniqueItems` from the grammar copy, and continue to fail visibly without repair or retry |
