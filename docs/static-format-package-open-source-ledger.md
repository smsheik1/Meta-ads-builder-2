...
Gemma 4 26B A4B remains a provisional Maker-analysis candidate, not the production selection. Three pinned v1.2 regression inferences completed in 11.955 to 20.276 seconds after warm-up, and the model recovered the central formula on all three references. Official validity was 0/3 because every response copied `$schema` into the closed result object. Diagnostic validity after removing only that metadata was 1/3: the comparison ad mis-modeled both repeated sets, LinkedIn left six OCR regions unowned, and the Meta Story passed 19/19 evidence ownership but under-decomposed assets. Fix the schema projection offline, then use genuinely new references for the next paid gate. Never ship multiple models as silent fallbacks.
