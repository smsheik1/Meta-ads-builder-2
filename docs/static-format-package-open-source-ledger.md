...
| Semantic reference analysis | [Gemma 4 31B IT](https://build.nvidia.com/google/gemma-4-31b-it) | Provisional semantic leader, pipeline not selected; [untouched holdout gate failed](./research-intake/static-reference-holdout-3-smoke-2026-07-10.md) | Tuned corpus retained 5/5 formula recovery and zero fake fragment Lists; the one valid holdout still grouped a seven-fragment headline into one Field and returned zero false Lists | Holdouts produced one partial valid response, one malformed prompt-only JSON response, and one NVIDIA 504; add actual `guided_json`, recalibrate scope/confidence, then use new holdouts |
...
Gemma 4 31B IT remains the provisional semantic benchmark leader, but the pipeline is not selected because the first untouched holdout gate failed. Before comparing another model, correct the benchmark integration to use provider-native `guided_json`; prompt-only malformed output is not a fair semantic-model comparison, and an HTTP 504 is a provider incident rather than semantic evidence. Then test new untouched holdouts. Only a repeatable semantic failure after that justifies one challenger comparison; never ship multiple models as silent fallbacks.
