...
flowchart LR
  ...
  Brief --> ThreeD["3D Breakdown story slate"]
  ThreeD --> Board["6-frame storyboard"]
  Board --> Anchors["2 production anchors"]
  Anchors --> Clips["2 animated clips"]
  Clips --> Voice["Fish narration"]
  Voice --> Final["20s MP4"]
...
| 3D media | `v3/features/formats/three-d-breakdown/*`, `v3/convex/threeDImages.ts` | Story slate, storyboard, anchors, clips, narration, and final video state |
...
| `three-d-breakdown` | `v3/features/formats/three-d-breakdown` | Evidence-grounded 3D product explainer with explicit paid media steps |
...
- `v3/features/formats/three-d-breakdown/generate.ts`,
  `v3/features/formats/three-d-breakdown/storyboardContracts.ts`, and
  `v3/convex/threeDImages.ts` own the current story-to-video assembly line.
  Preserve GLM-only failure behavior and explicit user clicks before paid media.
