...
| Platform chrome boundary | Analyze captured Story and feed ads | Status bars, account headers, native CTA stickers/buttons, captions, footers, and controls are excluded even when they contain the advertiser's brand or CTA text |
| UI-as-formula ambiguity | Upload a screenshot where native UI may itself be the desired visual formula | Wiggly proposes the creative bounds and asks the Maker to confirm whether the UI is intentional creative content; it does not silently decide from platform appearance alone |
