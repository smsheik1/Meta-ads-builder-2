...
| Provider-enforced schema | Inspect one semantic request and one Campaign-Play request | Each sends the pinned versioned JSON Schema through NIM `guided_json`, then passes local runtime validation; prompt-only JSON instructions do not qualify |
