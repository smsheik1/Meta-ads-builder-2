...
import {
  ...
  type IntroDuration,
  type SavedTemplate,
} from './createSavedDesigns';
import {
  appendMediaForRemotion,
  formatBytes,
  getMediaDurationSeconds,
} from './createExportMedia';
import { useCreateExportController } from './useCreateExportController';
import { CreateVoiceWizard } from './CreateVoiceWizard';
import {
  type CreativeBrief,
  type CreativeBriefTextKey,
} from './createVoiceScripts';
import { useCreateVoiceController } from './useCreateVoiceController';
import {
  DEFAULT_AUDIO_NAME,
  DEFAULT_AUDIO_URL,
  cleanCaptions,
...
} from './useCreateMediaController';
import {
  buildGeneratedAdApplication,
  normalizeCreativeBriefReceipts,
} from './createAdApplication';
import { CreateDesignLibrary } from './components/CreateDesignLibrary';
import { CreatePreviewStage } from './components/CreatePreviewStage';
...
const PERSONA_DECKS = [{ customer: 'Dental practice owner' }];
...
export type CreatePageRoute = 'builder' | 'create';
...
type CreatePageProps = {
  ...
  onRouteChange: (route: CreatePageRoute) => void;
  onOpenHome: () => void;
};

export function CreatePage({ appRoute, onRouteChange, onOpenHome }: CreatePageProps) {
  ...
  return (
    <div className="wiggly-builder-shell flex h-screen w-full flex-col overflow-hidden font-sans text-slate-900">
      ...
      <InteractiveTutorial enabled replayToken={tutorialReplayKey} />
    </div>
  );
}
