...
export function assertShareableAdScene(value: unknown): AdScene {
  ...
  if (scene.format !== "visualizer" && scene.format !== "jingle" && scene.format !== "text-message" && scene.format !== "brainrot" && scene.format !== "motion-story") throw new Error("Share scene format is not supported yet.");
