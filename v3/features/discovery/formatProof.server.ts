...
const formatConfigs: FormatProfileConfig[] = [
  ...
  },
  {
    ...
    promise: "Turn one supplied conversation audio track into a staged Dog-and-Bunny exchange with an approved exact-time role sheet, overlap-safe caption ownership, short progressive captions, three camera angles, and intentional emphasis motion.",
    lastUpdated: "August 2026",
    ...
    whatChanges: [
      "The user-supplied conversation audio",
    ...
    handoff: {
      requiredInputs: [
        "One local conversation audio file the user is allowed to use",
        "A transcript with speaker labels and timing when available, or permission for the agent to prepare and explicitly verify every beat",
        "A preferred packaged background, or permission to choose one",
      ],
      deliverables: [
        "One validated contiguous dialogue timeline",
        "One approved timed-role-sheet.md and checksum-bound approval receipt",
        "One inspected contact sheet and quality report",
        "One finalized vertical MP4 with the supplied audio",
      ],
      instructions: [
        "Download the runnable Repo, report KIT-MANIFEST.json's exact version, then run npm install, npm test, npm run check, and the free smoke test",
        "Copy the sample input for the new run, point it at the local user audio, and keep every complete spoken line in caption",
        "Run init, review timed-role-sheet.md and every extracted audio beat, record confirmedSpeaker and evidence, then run approve-script and validate",
        "Use mouth movement alone for neutral speech; add bounceAt only to an explicit punchline or strong reaction, with no more than two cues",
        "Use the packaged renderer, which automatically presents captions in one-to-three-word cards; do not rebuild or pre-split it",
        "Run render, inspect, and finalize in order, then return the playable MP4 and inspection evidence",
      ],
      estimates: [
        { label: "Timeline + speaker review", cost: "$0 provider cost", time: "about 5-15 min" },
        { label: "Local render + inspection", cost: "$0 provider cost", time: "about 2-5 min" },
      ],
      totalEstimate: "$0 provider cost, usually 7-20 min after the audio is supplied",
      output: "One 1080 × 1920 MP4 whose duration matches the supplied conversation audio",
      firstQuestion: "Attach the conversation audio you want the cat and bunny to perform. If you have a transcript or speaker labels, include those too.",
    },
  },
  {
    slug: "shaz-puppet-runtime",
    promise: "Download Format 0.3.0 to turn local audio into a Shaz talking scene with the Talk to Camera default or sequenced body language, five-mouth lip-sync from the bundled Cherry WASI cue engine, and four checksum-registered fixed backgrounds. Five trusted artist-authored actions remain visible in the secondary pose gallery.",
    lastUpdated: "August 2026",
    ...
    whatChanges: [
      ...
      "Sisters Room, Living Room, Photo Zone, or Pure White as the fixed background",
      "The selected actions and their order",
      "The final-pose hold durations",
      "The white separator durations",
      "The checksum-registered fixed background selected for the scene",
      "New semantic pose recipes after independent rig validation",
    ],
    handoff: {
      requiredInputs: [
        "One local audio file for the dialogue",
        "One built-in background ID; Sisters Room is the default",
      ...
      instructions: [
        "Download the runnable Repo, confirm Format 0.3.0, then run npm install, npm test, npm run check, npm run inspect:registry, and npm run smoke",
        ...
        "Keep shaz-body-language-performance-v1 body-language-only; Format 0.3.0 does not auto-apply lip-sync on that separate semantic path",
      ...
      estimates: [
        { label: "Sequence + validation", cost: "$0", time: "under 1 min" },
        { label: "Local rig render", cost: "$0", time: "about 1-5 min" },
        { label: "Inspection + review", cost: "$0", time: "about 1-3 min" },
      ],
      totalEstimate: "$0 provider cost, usually 2-8 min",
      output: "One horizontal 1280 × 720 MP4 over the selected registered background, plus inspection and delivery receipts; audio-backed sequences include AAC and local Cherry cue provenance",
      firstQuestion: "Attach the dialogue audio. Should Shaz use Talk to Camera or expressive registered actions, and should the fixed background be Sisters Room (default), Living Room, Photo Zone, or Pure White?",
    },
  },
  {
    slug: "jingle",
