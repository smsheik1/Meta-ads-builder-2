...
const getStoryboardStyleRules = (visualStyle: ThreeDBreakdownVisualStyle) => (
  visualStyle === "presenter-teardown-vsl"
    ? [
      ...
      "When the demo subject's face is visible, use a casual creator-ad 3D person with natural skin, visible eyes, optional plain cap or non-medical goggles on the head, everyday shirt, and product-demo posture; no smooth bald mannequin, blank anatomy model, test dummy, blue gloves, medical masks, lab technician, doctor, scientist, or PPE worker.",
      ...
      "Keep the demonstrator consistent across frames: same face, cap/goggles if used, shirt color, body scale, and relationship to the product.",
...
const createSelectedDirectionStoryboard = ({
  ...
  const framePlanText = frames.map((frame) => (
    `visual ${frame.visual}; camera ${frame.camera}; motion ${frame.motion}`
      ...
      "Keep the same demonstrator face, cap/goggles if used, shirt color, body scale, product silhouette, and product relationship across all six stills.",
