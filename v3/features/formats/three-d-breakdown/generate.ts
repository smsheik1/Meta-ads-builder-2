...
const parseStoryDirectionSlateOutput = (
  ...
  const directions = rawDirections.map((direction, index) => {
    ...
    if (
      ...
      && (
        builtInAllComponentsPattern.test(directionText)
        || separatelySoldAttachmentAggregationPattern.test(directionText)
        || /\bcold(?: therapy| attachment)?\b/i.test(directionText)
      )
    ) {
