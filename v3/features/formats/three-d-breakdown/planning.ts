...
const parseJsonObject = (value: string) => {
  ...
  const jsonText = trimmed.startsWith("{")
    ? trimmed
    : trimmed.match(/\{[\s\S]*\}/)?.[0] || "";
  if (!jsonText) throw new Error("3D Breakdown plan contains no JSON.");
  try {
    return JSON.parse(jsonText) as Record<string, unknown>;
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    throw new Error(`3D Breakdown plan contains malformed JSON: ${message}`);
  }
};
...
export const parseStoryDirectionSlateOutput = (
  raw: string,
...
export const parseStyleBScriptPlanOutput = (
  raw: string,
  evidenceItems: ThreeDBreakdownEvidenceItem[],
...
export const parseDirectorOutput = (
  raw: string,
  evidenceItems: ThreeDBreakdownEvidenceItem[],
  requestedCount: number,
  lockedStyleBScript?: ThreeDBreakdownLockedStyleBScript | null,
...
export const prepareThreeDBreakdownEvidence = (
  research: StoredWebsiteResearchResult,
  startedAt: number,
  storySubject?: ThreeDBreakdownResolvedStorySubject,
) => {
