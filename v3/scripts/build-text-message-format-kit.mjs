...
const formatRelative = path.join("public", "format-repositories", "text-message-v1");
const formatRoot = path.join(v3Root, formatRelative);
const stagingParent = path.join(v3Root, "tmp", "text-message-format-kit");
const kitName = "wiggly-text-message-format-kit";
...
for (const relativePath of [
  ...
  "features/scene",
  "remotion-entry",
  "scripts/text-message-format.ts",
  "tests/text-message-agent-runner.test.ts",
]) {
...
await writeFile(
  ...
  [
    "# Wiggly Text Message Format Kit",
    "",
    "Open `v3/SKILL.md` if you are an agent.",
    "Open `v3/public/format-repositories/text-message-v1/README.md` for the human quick start.",
