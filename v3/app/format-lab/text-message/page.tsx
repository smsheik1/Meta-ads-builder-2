...
const packagePath = path.join(process.cwd(), "public", "format-repositories", "text-message-v1");
...
const files = [
  ...
  ["prompts/conversations.md", "Conversation rules"],
  ["quality.json", "Acceptance checks"],
] as const;

export default function TextMessageFormatPage() {
      ...
      <div className="mx-auto max-w-6xl space-y-7 px-5 py-7">
        <section className="rounded-lg border border-slate-300 bg-white p-6 shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-5">
            ...
            <Button asChild data-testid="download-text-message-kit">
              <a href="/format-repositories/text-message-v1/downloads/wiggly-text-message-format-kit.zip" download>
        ...
        <section data-testid="text-message-golden">
          ...
          <div className="mt-4 grid gap-5 lg:grid-cols-[minmax(0,420px)_1fr]">
            {goldens.examples.map((example) => (
              <article key={example.id} className="overflow-hidden rounded-lg border border-slate-300 bg-white shadow-sm">
                <img
                  ...
                  src={`/format-repositories/text-message-v1/${example.imagePath}`}
            ...
            <article className="rounded-lg border border-slate-300 bg-white p-5 shadow-sm">
              <h2 className="font-bold">Believable by default</h2>
              <ul className="mt-3 space-y-3 text-sm text-slate-700">
                <li>The agent asks for one website and handles the rest.</li>
                <li>Six buyer moments create useful options without extra questions.</li>
                <li>Hard text budgets keep every conversation on one screen.</li>
                <li>The brand appears once, like a recommendation between friends.</li>
        ...
        <section data-testid="text-message-pipeline">
        ...
        <section className="grid gap-5 lg:grid-cols-2">
          <article className="rounded-lg border border-slate-300 bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 overflow-auto rounded-md bg-slate-950 p-4 font-mono text-xs leading-6 text-slate-100">
              npm run format:text-message -- init --run=&lt;id&gt; --url=&lt;url&gt;<br />
              npm run format:text-message -- prompt --run=&lt;id&gt;<br />
              npm run format:text-message -- validate --run=&lt;id&gt;<br />
              npm run format:text-message -- render --run=&lt;id&gt;<br />
          ...
          <article className="rounded-lg border border-slate-300 bg-white p-5 shadow-sm">
            ...
            <ul className="mt-3 space-y-2 text-sm text-slate-700">
              ...
              <li>Unsupported proof cannot enter a conversation.</li>
              <li>Duplicate angles fail validation.</li>
              <li>One-sided or overlong threads fail validation.</li>
