...
import { isBestSellerProduct, sortReviewProductsForSelection } from "@/features/formats/reviews/productSelection";
import type {
...
const subjectOptions: Array<{
  ...
  { kind: "product", title: "A specific product", description: "Lock the script, visuals, and CTA to one real item." },
  { kind: "brand", title: "Tell the brand story", description: "Explain why this brand exists using real website evidence." },
  { kind: "customer-problem", title: "Expose a customer problem", description: "Find the hidden friction buyers have not noticed." },
  { kind: "custom", title: "Start with my angle", description: "Give Wiggly your premise, audience, or campaign moment." },
];
...
export function CreateThreeDBreakdownSubjectPicker({
  ...
  const products = useMemo(() => sortReviewProductsForSelection(catalog?.products || []), [catalog?.products]);
    ...
    <section className="rounded-[1.6rem] border border-slate-200 bg-white p-4 shadow-xl shadow-slate-950/6" data-three-d-subject-picker="true">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <h3 className="mt-1 text-lg font-black text-slate-950">What should this video be about?</h3>
          <p className="mt-1 text-[11px] font-bold leading-4 text-slate-500">Choose the subject before Wiggly writes five concepts.</p>
        </div>
        ...
        {subjectOptions.map((option) => {
          ...
          return (
            <button
              ...
              className={`rounded-2xl border p-3 text-left transition ${selected ? "border-slate-950 bg-slate-950 text-white" : "border-slate-200 bg-slate-50 text-slate-950 hover:border-slate-300"}`}
              data-three-d-subject-option={option.kind}
            >
              <p className="text-sm font-black">{option.title}</p>
              <p className={`mt-1 text-[11px] font-semibold leading-4 ${selected ? "text-white/70" : "text-slate-500"}`}>{option.description}</p>
            </button>
          );
        })}
      </div>
      ...
      {kind === "product" ? (
        <div className="mt-3 rounded-2xl border border-slate-200 bg-slate-50 p-3">
          {products.length ? (
            <Sheet>
              ...
              <SheetContent className="flex w-full flex-col border-slate-200 bg-white p-0 sm:max-w-3xl">
                <SheetHeader className="border-b border-slate-200 px-6 py-5 text-left">
                  ...
                  <SheetDescription className="font-semibold leading-6 text-slate-500">Every product found on this website is here. Your choice stays locked through the script, images, and CTA.</SheetDescription>
                </SheetHeader>
                  ...
                  {filteredProducts.map((product) => {
                    ...
                    return (
                      <button
                        ...
                        disabled={!usable}
                        className={`relative min-h-[18.5rem] overflow-hidden rounded-2xl border bg-white text-left shadow-sm transition ${selected ? "border-slate-950 ring-2 ring-slate-950" : "border-slate-200 hover:border-slate-300"} ${usable ? "" : "cursor-not-allowed opacity-55"}`}
            ...
            <p className="text-xs font-bold leading-5 text-slate-500">No product catalog was found. Choose a brand, customer problem, or custom story instead.</p>
          )}
        </div>
      ) : null}
