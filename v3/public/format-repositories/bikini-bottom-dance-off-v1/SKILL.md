...
6. If Fish clips are missing, confirm `FISH_STUDIO_APIKEY` without printing it, show the eight-line script and current estimate, obtain approval, then run `render --approve-provider`. Every character admitted by the input contract has a packaged reference preset. Accepted dialogue is cached. Do not replace the Character Dance Lab renderer or retargeter.
