...
4. If the desired motion is absent, have the operator download one Mixamo Collada file with skin and run `node runner.mjs import-motion`. Do not automate Mixamo's private browser endpoints or package the source DAE.
...
8. Finalize only with `--human-review=pass` after a person approves the rendered proof.
