...
Export `FISH_STUDIO_APIKEY` locally; never add an env file to the kit. The 19 approved transferable fallback references in `assets/voice-presets.json` are packaged with the Format and bound to the selectable input-contract roster. Wiggly operator runs may also set `BIKINI_BOTTOM_DANCE_OFF_OLAF_VOICE_ID` to use the approved private Olaf clone; the runtime prefers that value and invalidates dialogue cached from the public fallback. The variable name is public, but its value, the private model ID, and the supplied reference audio are never packaged. Recipients without access to Wiggly's private clone continue to use the packaged Olaf fallback instead of receiving a broken kit. Agent P, Man Ray, and Batman Beyond are deliberately excluded from spoken episodes instead of receiving guessed voices. The public Repo page makes no provider call: its Play Voice controls consume only the packaged MP3s and hashes in `assets/voice-previews/manifest.json`, and only one preview may play at a time. Missing or stale episode dialogue still requires explicit `--approve-provider`; accepted clips are cached with non-secret receipts in the ignored run folder.
