...
Set `FISH_STUDIO_APIKEY` locally. The approved reference presets for SpongeBob, Patrick, Mr. Krabs, Squilliam, Squidward, and Sandy are packaged with the Format. The other motion-ready characters remain unavailable for spoken episodes until their Fish Audio audit is resolved and approved. Missing or stale dialogue requires explicit `--approve-provider`; accepted clips are cached with non-secret receipts in the ignored run folder.
