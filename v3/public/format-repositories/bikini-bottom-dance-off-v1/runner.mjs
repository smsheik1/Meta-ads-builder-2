...
async function validateInput(inputPath, { receiptPath } = {}) {
  ...
  const errors = [];
  const allowed = new Set(["title", "songTitle", "songFile", "songExcerptStart", "characters"]);
...
async function renderRun() {
  ...
  await composeRun({ input, runDirectory: directory, outputPath: path.join(directory, "render.mp4") });
...
async function checkRepo() {
  ...
  for (const file of ["runtime/compose.mjs", "runtime/analyze-audio.mjs", "runtime/inspect.mjs", "../mixamo-character-motion-v1/runtime/renderer/app.js"]) await access(path.join(root, file));
