...
- `fonts/PatrickHand-Regular.ttf`: Patrick Hand from Google Fonts, distributed under `fonts/OFL.txt`.
- `poses/*.png`: recurring pose pack extracted from the user-owned source supplied for this proof. `pose-hashes.json` locks the approved bytes so an agent cannot silently replace the character.
