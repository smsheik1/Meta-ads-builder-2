...
fn read_tgtb_domain(input: &mut impl Read) -> Result<(f32, f32), ReadError> {
    ...
    if unknown != 0 {
        ...
        trace!("unexpected tGTB bytes after domain start: {unknown:16x}");
    }
    ...
    if unknown != 0 {
        // TODO figure out what these are
        trace!("unexpected tGTB bytes after domain end: {unknown:16x}");
    }
    ...
    let definition = match defn_type {
        ...
        0x01 => {
            ...
            if trailer != trailer_read {
                trace!(
                    "unexpected tGTB definition trailer: {:?}. ignoring!",
