    ...
    R: Read,
{
    ...
    let tags = read_tags(&mut PeekReader::new(input)?)?;
...
fn read_tags<R: Read>(input: &mut PeekReader<R>) -> Result<Vec<FileData>, ReadError>
{
    ...
    R: Read,
{
    ...
    match FileTag::try_from(tag) {
        ...
        Ok(FileTag::MainData) => {
            ...
            Ok(FileData::Main(read_tags(&mut PeekReader::new(io::Cursor::new(data))?)?))
        }
