...
3. Run `init`. It extracts one local WAV clip per timeline beat and creates `script-review.json` plus the readable `timed-role-sheet.md`.
4. Draft the complete timed role script. Every active beat contains exactly one of `caption` for words or `vocalization` for a named nonverbal sound such as an emotional gasp or shriek; silence uses `speaker=none`. Split words and nonverbal performance into separate contiguous beats. Keep an elongated phrase, internal dramatic pause, and trailing word assigned to the same speaker through the last audible word; never create a speaker boundary merely because the delivery slows down, and never extend that beat past the next character's first audible word. Automated transcription, diarization, dialogue context, and cameras may propose a draft but never approve character roles. A captioned `speaker=both` beat also requires `captionSpeaker=cat`, `bunny`, or `both` so caption ownership is never inferred from simultaneous mouth motion.
5. Show `timed-role-sheet.md` to the user before rendering. It contains every exact time range, spoken line, named nonverbal vocalization, silence, Dog/Bunny assignment, caption owner, confirmed overlap, and review clip in order. Incorporate corrections, regenerate with `review-script`, then set `script-review.json` approval only after the user explicitly approves the complete sheet. Confirm every beat through the strongest evidence channel available. For `local-audio-analysis`, use `voiceCharacterMap` and `detectedVoices` only when anonymous clusters genuinely represent distinct stable speakers; do not force one voice-to-character map onto a multi-character performance. `both` means proven simultaneous speech only. Record the approval basis, approver, and note.
...
12. Directly watch the final MP4 and inspect the approved timed role sheet and contact sheet before returning it. If sound is perceptible, review it too; otherwise verify the audio stream, duration, codec, and level and explicitly state that intelligibility was not scored.
