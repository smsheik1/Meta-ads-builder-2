...
The blue dog is the grounded, wise lead and uses the legacy runtime ID `cat`; the pink bunny is the secondary questioner/foil. Preserve that casting for the complete episode.
...
4. Confirm every beat through the strongest explicit evidence channel available. Prefer direct clip listening; otherwise require documented local audio analysis, a user-provided label, a checksum-matched documented reference video, or confirmed silence. For `local-audio-analysis`, define each stable detected voice ID once in `voiceCharacterMap`, list the beat's `detectedVoices`, and add an `evidenceNote` naming the ASR/diarization basis. Map the user-identified grounded/wise lead voice to `cat` (the blue dog) and the secondary foil voice to `bunny`; never recast a detected voice at a caption or apparent turn boundary. Set each `confirmedSpeaker` and `evidence` in `speaker-review.json`, disclose any perception limitation, and never infer a speaker from caption text or camera alone. `both` means proven simultaneous speech only: include mapped dog and bunny voices, set `overlapConfirmed: true`, and document the overlap in `evidenceNote`. Never use `both` for uncertainty or alternating voices; stop for stronger evidence or split alternating turns into contiguous single-speaker beats.
