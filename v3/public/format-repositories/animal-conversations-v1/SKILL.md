...
6. Leave mouth pause detection to the official renderer. It analyzes the supplied audio locally, closes the verified speaker's mouth only for sustained low-energy pauses, and suppresses sub-three-frame closures; do not add lip-sync cues, viseme tools, thresholds, or model dependencies.
7. Keep each beat's complete spoken line in `caption`; the official renderer—not the agent—turns it into progressive one-to-three-word cards.
...
9. Run `apply-speakers`, then `validate`. Validation rejects missing or stale speaker confirmation and invalid bounce cues.
