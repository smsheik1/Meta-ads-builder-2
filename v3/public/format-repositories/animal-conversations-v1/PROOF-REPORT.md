...
- Format-specific casting rule: the blue dog uses the legacy runtime ID `cat` and is the grounded, wise lead; the pink bunny is the secondary questioner/foil. That role declaration is fixed for this Format rather than promoted into the shared cross-Repo standard.
...
- Guardrail and evidence: a regression test tries to map `voice_0` to cat early and bunny later and must fail. All automatic gates pass, including `stableVoiceCastingConfirmed`. The 2-fps ending sheet shows the dog through `If you made a mistake`, `Oh! You better speak up`, `No, if I make a mistake`, `absolutely devastating`, and `Why?`. Corrected output SHA-256: `b58823b454e3fdf89eb16837e0372413b54c1324d64502ff40a710ecb15d116b`.

## Complete role-script approval correction

- Observed failure: even after stable anonymous voice casting was added, the unseen episode still assigned several character performances incorrectly. The missed dog gasp/shriek around 13 seconds was treated as speech, the bunny's rapid `No, no, no!` reassurance was split across characters, and the ending roles were inferred from imperfect clustering instead of the user's intended Dog/Bunny performance.
- Root cause: transcription and diarization drafts were allowed to become executable after per-beat evidence entry without requiring the user to see and approve one complete role script. That was too weak for a single performer acting multiple characters, short interjections, emotional nonverbal sounds, and clustering errors. A stable anonymous voice ID is useful evidence only when it genuinely represents one stable speaker; it is not a character oracle.
...
- Approved unseen-audio proof: the audio SHA-256 remains `53b2be558855a2ebacb1f3c1336b721b6ae37735ced25547a14cf2c7724344d5`. Its approved 15-beat script contains 13 spoken beats, the dog's uncaptained emotional gasp/shriek at 13.57–15.235, the bunny's uninterrupted `No, no, no!` at 15.235–19.30, one silent beat, the dog's `No, if I make a mistake, it's absolutely devastating.`, and the bunny's final `Why?`. It has zero `both` beats and zero bounce cues. The approval/script hash is `12799ab3f6629e6a2f34509836c46dcb955ae97893face9039d22644757a342d`.
- Render and inspection evidence: the finalized v15-renderer proof passes every gate at 1080x1920, 24 fps, H.264/AAC, 30.041667 seconds, and -20.3 dB mean audio. Its output SHA-256 is `9c4aefc071831b73c37a78a88bdc712587687f850f16767bf37cfa5f6451192e`. An original-resolution 15-panel role sheet confirms the exact Bunny/Dog/Bunny/Dog/Bunny/Dog/Bunny/Bunny/Dog/Bunny/Bunny/Dog/pause/Dog/Bunny sequence and all three cameras. Audio intelligibility was not directly scored by this agent; the user's explicit script approval is the role authority.
...
`npm run build:kit` produced a stable version 0.11.0 ZIP without `node_modules`, generated runs, Cargo targets, raw runtime audio, standalone script-review clips, or download artifacts. The intentionally distributable approved proof MP4 retains its soundtrack. The archive includes explicit entrypoints for Codex, Claude Code, Cursor, and other coding agents plus a versioned `KIT-MANIFEST.json`. A fresh extraction under `/private/tmp` completed `npm install`, all twenty-seven Node tests, the Rust decoder test, `npm run check`, and the full free smoke command successfully. The packaged smoke generated and applied a current complete-script approval receipt before rendering, including explicit overlap proof for its mechanics-only simultaneous beat; inspection confirmed all three camera/character modes, the repaired bunny head, the cue-capable renderer, progressive bottom-third captions without a background panel, independent conversational blinks, audio-responsive pause closures, envelope-paced mouth changes, the overlap-only `both` guardrail, stable voice-casting enforcement for applicable local analysis, and the new spoken/nonverbal/silent approval counts. `zip -X` removes nondeterministic filesystem metadata; two consecutive builds produced the same archive SHA-256. The final implementation retained one renderer, one runtime, and no transcription, diarization, viseme, or provider dependency.
