...
const [manifest, pkg, format, files] = await Promise.all(["KIT-MANIFEST.json", "package.json", "format.json", "release-files.json"].map(async (file) => JSON.parse(await readFile(path.join(root, file), "utf8"))));
if (manifest.version !== pkg.version || manifest.version !== format.version || manifest.kit !== pkg.name || format.id !== "repo-builder") throw new Error("Builder identity/version mismatch");
