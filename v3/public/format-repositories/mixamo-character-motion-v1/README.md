...
The August 2026 character import audited 51 user-supplied archives one character at a time. Four archives were already represented, eleven new rigs passed both Silly Dancing and Macarena visual proofs, and 36 remain rejected with exact rig or rendering reasons in `assets/character-import-audit.json`. Aqua's supplied skinned glTF was converted once, offline, into the same packaged Collada contract; Ratchet's anonymous skeleton was identified from its hierarchy and skin influences. The runtime still has one loader and no added conversion dependency. Rejection never weakens the runtime contract.
...
- The lab exposes exactly 25 bundled starter motions plus any operator-imported user motions, and fifteen motion-ready characters.
