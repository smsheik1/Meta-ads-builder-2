...
The August 2026 character import audited 51 user-supplied archives one character at a time. Four archives were already represented, thirteen new rigs passed both Silly Dancing and Macarena visual proofs, and 34 remain rejected with exact rig or rendering reasons in `assets/character-import-audit.json`. Aqua's supplied skinned glTF was converted once, offline, into the same packaged Collada contract; Ratchet, Larry, and Man Ray's anonymous skeletons were identified from hierarchy and skin influences. The runtime still has one loader and no added conversion dependency. Rejection never weakens the runtime contract.
