...
The August 2026 character import audited 51 user-supplied archives one character at a time. Four archives were already represented, eighteen new rigs passed both Silly Dancing and Macarena visual proofs, and 29 remain rejected with exact rig or rendering reasons in `assets/character-import-audit.json`. Aqua's supplied skinned glTF was converted once, offline, into the same packaged Collada contract; Ratchet, Larry, and Man Ray's anonymous skeletons were identified from hierarchy and skin influences; Batman Animated and Batman Beyond's omitted Collada skeleton roots were repaired while their malformed oversized cape instances were excluded; and Dr. Doofenshmirtz, Ferb, and Phineas's authored pelvises were preserved by retargeting their true parent roots. The runtime still has one loader and no added conversion dependency. Rejection never weakens the runtime contract.

## Add another character

1. Work on exactly one source archive. Prefer a skinned Collada rig with named bones; a static mesh is not motion-ready. A skinned glTF/FBX may be converted offline into Collada when the conversion is bounded and visually proven, but never add a runtime loader for one character.
2. Copy only that character's production `.dae` and referenced textures into `assets/characters/<id>/`.
3. Add one declarative entry to `assets/character-packs.json`: model path, display scale, optional pitch/yaw, semantic `boneMap`, root, feet, leg chains, and protected face or overlay bones. Use `pitch: 0` only for a Y-up Collada source that omits the usual `Z_UP` declaration. Display-only repairs also stay declarative: use `alphaCutoutMaterials` for hard-edged eye/lash transparency, `materialOpacities` for blended glass, `hiddenMeshes` only for authored alternate shells, and `restPoseAdjustments` only after visually calibrating existing rig controls. Do not add character-specific renderer code.
