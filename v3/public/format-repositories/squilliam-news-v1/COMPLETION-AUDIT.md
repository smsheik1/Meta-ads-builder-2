...
| Create two meaningfully different promotional proofs through the current runtime | Pending v0.2 refresh | The v0.1 We The Artists and Wiggly Format Lab proofs share historical runtime hash `cc7fe1bfb8dd651063b247399f2103530bfe988f02a04149a8250c0fb2394558`. Adding selectable character packs changes the official runtime, so those videos are references rather than current finalization evidence. |
| Inspect real rendered media | Pass for character-pack scope | All four v0.2 presenters have playable smoke MP4s, contact sheets, content hashes, media hashes, and visual admission checks. Full 30-second promotion evidence remains historical v0.1 evidence until refreshed. |
...
| Finalize human-approved real media | Historical v0.1 pass | We The Artists has matching v0.1 `final.mp4`, `quality-report.json`, and `finalization.json`; the blind package run independently finalized the same video hash. A v0.2 full render requires fresh evidence. |
| Finalize the second creative proof | Pending v0.2 run and human review | The v0.1 Wiggly Format Lab review passed its automatic checks, but the renderer fingerprint is now stale. A fresh v0.2 run ID is required; the old attempt limit must not be bypassed. |
...
The character-import expansion is complete. The broader full-Repo goal remains active for fresh v0.2 promotional proofs and human finalization of the second creative proof.
