...
The checked-in compatible importer now requires the actual archive during authoring. It verifies the archive checksum, selected Xstage member, every compiled TVG source member, mapped transform parents, pivots, stage field grid, and the complete renderer-consumed registration for each drawing variant before it commits a recipe/audit pair. Its recovery journal prevents an interrupted two-file update from being mistaken for one coherent import.
