...
5. Treat drawing IDs as scene-local. Reuse canonical art only after the normalized source drawing matches it; source-bind both missing IDs and same-number drawings whose artwork differs. Compile those TVGs into a hash-namespaced asset directory and preserve their per-asset Xstage provenance.
