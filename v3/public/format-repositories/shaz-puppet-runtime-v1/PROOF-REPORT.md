...
## Format 0.2.1 Talk to Camera boundary

`sequencePreset: "talk-to-camera"` is a composition preset, not a new pose recipe. Init measures the supplied audio and records `durationFrames`; validation resolves exactly one gap-free `neutral-listening` body entry for that duration and requires checksum-bound Cherry cues. The user supplies no `sequence`, `holdFrames`, or manual frame calculation. Validation, render, inspection, and delivery receipts carry the preset ID, and the contact sheet samples real mouth-change frames.

The preset reuses the existing one-frame authored neutral anchor. It does not add body controls, props, camera motion, or a renderer branch. Explicit Lego sequences remain backward compatible and keep their 120-frame per-entry hold guardrail.
