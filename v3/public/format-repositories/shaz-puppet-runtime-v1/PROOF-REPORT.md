...
- Automated tests: 219/219 passing, including reproducible package bytes, bundled-engine parity, transcript and cue provenance, tamper rejection, transcript-anchored choreography, local-only audio ingress, cache-link rejection, audio-backed rendering, fixed-stage framing, the duration-derived Talk to Camera preset, the exact four-background registry, compatible-Xstage source binding, streamed source-archive and exact-member verification, scene-local same-number drawing collisions, mixed-palette normalization, parent-graph/pivot/field-grid compatibility, Harmony constant-velocity holds and rejection of unsupported curves, exact compiler/importer node boundaries, recoverable recipe/audit and asset-tree commits, the canonical reconstruction-tutorial contract, lease-safe compatible-asset registration, and a smoke-fixture guard that excludes needs-review poses
