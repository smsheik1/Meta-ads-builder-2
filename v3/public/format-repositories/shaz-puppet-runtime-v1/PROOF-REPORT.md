...
For `anatomy-v8-release`, Codex acted under that standing delegation, watched the exact output checksum completely at normal speed through `ended=true`, sampled every action throughout playback, inspected the contact sheet, and recorded the review under its own name. This does not claim that the user personally watched or approved the release artifact.

The separate public ten-action golden still records `userVisualApproval: pending`, so the rich Repo page remains testable but intentionally stays off the main Discovery shelf. That publication gate is independent of this completed, reusable package.
